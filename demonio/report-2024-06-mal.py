...
def printStamps(modelo1_ticket, modelo2_ticket, id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos):
	...
	if totalImporte > 349:
		vecesMod1Especial = 4
		vecesMod2Especial = 4
	elif totalImporte > 299:
		vecesMod1Especial = 3
		vecesMod2Especial = 3
	elif totalImporte > 199:
		vecesMod1Especial = 2
		vecesMod1Especial = 2
	#elif totalImporte > T1especial:
	elif totalImporte > 99:	
		vecesMod1Especial = 1
		vecesMod1Especial = 1
...
#----inicio ------------- AÑO 2024= SI = IMPORTE MAYOR 100€ 1 TIRA ESPECIAL, 200€ 2 TIRAS, 300€ 3 TIRAS y 350€ 4 TIRAS
			...
			if vecesMod1Especial > 0:
				...
				#	for x in xrange(0,vecesEspecial):
					...
					genStampI(modelo1_ticket, "Tarifa A", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cte[modelo-1])
					id_producto = int(id_producto) + 1
					cte[modelo-1].showPage()
					genStampI(modelo1_ticket, "Tarifa A3", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cte[modelo-1])
					id_producto = int(id_producto) + 1
					...
					id_producto = int(id_producto) + 1
					cte[modelo-1].showPage()
					genStampD(modelo2_ticket, "Tarifa A", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cte[modelo-1])
					id_producto = int(id_producto) + 1
					cte[modelo-1].showPage()
					genStampD(modelo2_ticket, "Tarifa A3", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cte[modelo-1])
					id_producto = int(id_producto) + 1
...
# ticket caja inicio - anulado TAIPEI O INTERNACIONAL o FERIAS PEQUEÑAS
