...
def genStampI(tarifa, fecha, evento, codigo, c):
	pdfmetrics.registerFont(TTFont('FranklinGothic', 'franklin_gothic.ttf'))
	pdfmetrics.registerFont(TTFont('FranklinGothicBold', 'franklin_gothic_bold.ttf'))
	pdfmetrics.registerFont(TTFont('FranklinGothicCondensed', 'franklin_gothic_condensed.ttf'))
	...
	#new
	#drawfondoeti("fondoetiqueta.png", 55*mm, -45*mm, page_width, c) # irun p  ANTES -39 / VALENCIA= -41
	drawfondoeti("fondoetiqueta1.png", 55*mm, -41*mm, page_width, c) # ok PLAZA MAYOR 2022= -38
	#drawText("Filatelia", "FranklinGothic", 12, 1.5*mm, 19.5*mm, c) 
...
def genStampD(tarifa, fecha, evento, codigo, c):
	...
	#new
	#drawfondoeti("fondoetiqueta.png", 55*mm, -45*mm, page_width, c) # irun p ANTES -39 / VALENCIA= -41
	drawfondoeti("fondoetiqueta2.png", 55*mm, -41*mm, page_width, c) # ok PLAZA MAYOR 2022= -38
	#drawText("Filatelia", "FranklinGothic", 12, 1.5*mm, 19.5*mm, c) 
...
def printStamps(id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos):
	...
	for index, item in enumerate(items):
#----inicio ------------- AÑO 2022= SI <> CH IMPRIME SELLOS CON fecha_sello y evento_sello Y LOGO DE FONDO
		if item["cantidad"]>0 and  str(nombre_maquina[0:2]) != "CH":
			...
			if productos[index]["modo"] == "S":
				...
				for x in xrange(0,item["cantidad"]):
					if modelo==1:
						genStampI(productos[index]["nombre_ticket"], fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cs[modelo-1][tarifa])
				...
				if productos[index]["idProducto"] == "tarifaAT1" or productos[index]["idProducto"] == "tarifaAT2":
					...
					for x in xrange(0,item["cantidad"]):
						if modelo==1:	
							genStampI("Tarifa A", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
					...
					for x in xrange(0,item["cantidad"]):
						if modelo==1:
							...
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampI("Tarifa A2", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampI("Tarifa B", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampI("Tarifa C", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()	
						else:
							genStampD("Tarifa A", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							...
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()		
#----inicio ------------- AÑO 2022= SI = CH IMPRIME SELLOS SIN LOGO DE FONDO
		elif item["cantidad"]>0 and  str(nombre_maquina[0:2]) == "CH":
			modelo = int(productos[index]["idProducto"][-1:])
