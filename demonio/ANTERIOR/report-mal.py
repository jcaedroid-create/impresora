...
def genStampI(tarifa, fecha, evento, codigo, c):
	...
	#drawfondoeti("fondoetiqueta.png", 55*mm, -45*mm, page_width, c) # irun p  ANTES -39 VALENCIA
		#drawfondoeti('"' + modelo1_ticket + '.png' + '"', 55*mm, -38*mm, page_width, c)
...
def genStampD(tarifa, fecha, evento, codigo, c):
	...
	#drawfondoeti("fondoetiqueta.png", 55*mm, -45*mm, page_width, c) # irun p ANTES -39 VALENCIA
			#drawfondoeti('"' + str(modelo2_ticket) + '.png' + '"', 55*mm, -38*mm, page_width, c)
...
def printStamps(modelo1_ticket, modelo2_ticket, id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos):
			...
			if productos[index]["modo"] == "S":
				...
				for x in xrange(0,item["cantidad"]):
					if modelo==1:
						...
						genStampD(productos[index]["nombre_ticket"], modelo2_ticket, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cs[modelo-1][tarifa])
						id_producto = int(id_producto) + 1
						cs[modelo-1][tarifa].showPage()
...
def afkarPrint(items, precios, id_cliente, id_producto, fecha_sello, evento_sello, fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, feria, lugar, empresa, cif, cp, l1, l2, l3):
	...
	try:
		...
		print e
	try:
		printStamps(modelo1_ticket, modelo2_ticket, id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos)
	except Exception as e:
		print e
