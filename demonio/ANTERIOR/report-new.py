...
def printStamps(id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos):
	...
	for index, item in enumerate(items):
		...
		if item["cantidad"]>0 and str(nombre_maquina[0:2]) != "CH":
			...
			if productos[index]["modo"] == "S":
				...
				if tarifa == "A":
					...
					tarifa = 3
		#----inicio if------------- AÑO 2020= SI <> CH IMPRIME SELLOS CON fecha_sello y evento_sello SI ES CH dejar en blanco			
		...
		#------else----------- AÑO 2020= SI <> CH IMPRIME SELLOS CON fecha_sello y evento_sello SI ES CH dejar en blanco

		#----fin else---------- AÑO 2020= SI <> CH IMPRIME SELLOS CON fecha_sello y evento_sello SI ES CH dejar en blanco
...
# nuevo para COPIAS TICKET TIRAS "ticketTira.pdf":  def printTicketsTIRAS(fecha_ticket, modo_ticket,... DEFINIR EN servidor-ws.py longitud fija.
...
def printTickets(fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3):
	...
	def printTicketsTira(fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3):
	...
	page_height = 126 * mm
	# ------------------ page_height = 165 * mm con LINEA QUITADA 148
	ctira = canvas.Canvas("tickettira.pdf", pagesize=(page_width, page_height))
	#elnombre_maquina = ''
	#elnombre_maquina = 'CH' + str(year_maquina)[-2:]

	# if nombre_maquina (es distinto a) ("CH" + year_maquina) (ENTONCES IMPRIME TICKETS POR CADA TIRA)
	itemsColeccion = []
	for item in items:
	...
	for index, item in enumerate(itemsColeccion):
		...
		# if items[index]["cantidad"]>0 and productos[index]["modo"] == "T" and modo_ticket == "Factura Simplificada":
		if items[index]["cantidad"]>0 and productos[index]["modo"] == "T" and  str(nombre_maquina[0:2]) != "CH":	
			...
			for x in xrange(0,items[index]["cantidad"]):
				print item
				genTicket(fecha_ticket, modo_ticket, modelo1_ticket, modelo2_ticket, itemsColeccion, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3, page_height, page_width, ctira)
				ctira.showPage()
...
def afkarPrint(items, precios, id_cliente, id_producto, fecha_sello, evento_sello, fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, feria, lugar, empresa, cif, cp, l1, l2, l3):
	...
	try:
		...
		print e
	try:
		printTicketsTira(fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3)
	except Exception as e:
		print e
	try:
