...
def genTicketCaja(fecha_ticket, modo_ticket, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3, page_height, page_width, c):
	...
	drawfondot("fondoticketcop.png", 70*mm, c1*mm, page_width, c) 
                              #110
	drawTitle(feria, "FranklinGothicBold", 12, c2*mm, page_width, c)  
	drawTitle(lugar, "FranklinGothicBold", 10, c2*mm - 6*mm, page_width, c)                #125 
	drawTitle(empresa, "FranklinGothicBold", 7.5, c2*mm - 10*mm, page_width, c)            #125
	...
	for index, item in enumerate(items):
		if item["cantidad"]>0:
			...
			drawItemCaja(y*mm, modelo_ticket + "Mortadelo y Fimelemón Master Set", "1", "28.60€", "28.60€", c)
	...
	# drawLine(5*mm, 38*mm, page_width-2*5*mm, c) 
		# drawMotives(34*mm, "Feria - Cibeles", "Feria - Caballo", "0361", "0480", "0364", "0483", page_width, c)
	drawText("Total:     1", "FranklinGothicCondensed", 8, 40*mm, y*mm - 6*mm, c)                        #90 ESPAÑOL                      #90 ESPAÑOL
	drawText("28.60€", "FranklinGothicCondensed", 8, 65*mm, y*mm - 6*mm, c)
...
def genStampE3(tarifa, fecha, evento, codigo, esp, c):
	pdfmetrics.registerFont(TTFont('FranklinGothic', 'franklin_gothic.ttf'))
	pdfmetrics.registerFont(TTFont('FranklinGothicBold', 'franklin_gothic_bold.ttf'))
	pdfmetrics.registerFont(TTFont('FranklinGothicCondensed', 'franklin_gothic_condensed.ttf'))
			...
			if TEmod1 == "S" or TEmod1 == "s":	
				...
				#	for x in xrange(0,vecesEspecial):
					...
					#	cte[modelo-1].showPage()
						...
						cte[modelo-1].showPage()
					# ETIQUETA 2
						genStampE2("Tarifa A3", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), "-E", cte[modelo-1])
						id_producto = int(id_producto) + 1
						cte[modelo-1].showPage()
					...
					#	cte[modelo-1].showPage()
					# ETIQUETA 3
						genStampE3("Tarifa A3", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), "-E", cte[modelo-1])
						id_producto = int(id_producto) + 1
						cte[modelo-1].showPage()
...
# ticket caja inicio - anulado TAIPEI O INTERNACIONAL o FERIAS PEQUEÑAS _______________________________
	if ImprimeCopiaTicket == "S" or ImprimeCopiaTicket == "s":
		genTicketCaja(fecha_ticket, "", "", "", items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3, page_height, page_width, c)
		c.showPage()
