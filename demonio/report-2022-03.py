...
def drawItemCaja(y, itemName, quantity, price, total, c):
	drawText(itemName, "FranklinGothicBold", 8, 5*mm, y, c)
	drawTextRight(quantity, "FranklinGothicBold", 8, 35*mm, y, c)
...
def genTicket(fecha_ticket, modo_ticket, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3, page_height, page_width, c):
	...
	drawText("Cant.", "FranklinGothicCondensed", 8, 45*mm, c4*mm, c)                        #90
	...
	print id_cliente
	#----------------- TRADUCCION 3/3
	drawTitle("" + nombre_maquina +( " - Sesión: ") + id_cliente, "FranklinGothicCondensed", 9, 20*mm, page_width, c)
...
def genTicketCaja(fecha_ticket, modo_ticket, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3, page_height, page_width, c):
	...
	drawfondot("fondoticketcop.png", 20*mm, c5*mm - 10*mm, page_width, c) 
	    #125
...
def genStamp(tarifa, fecha, evento, codigo, c):
	...
	drawfondoeti("fondoetiqueta.png", 55*mm, -41*mm, page_width, c)
	...
	drawText(codigo, "FranklinGothic", 6, 1.5*mm, 12*mm-9*mm, c)

def genStampD(tarifa, fecha, evento, codigo, c):
	pdfmetrics.registerFont(TTFont('FranklinGothic', 'franklin_gothic.ttf'))
	pdfmetrics.registerFont(TTFont('FranklinGothicBold', 'franklin_gothic_bold.ttf'))
	pdfmetrics.registerFont(TTFont('FranklinGothicCondensed', 'franklin_gothic_condensed.ttf'))
	...
	#new
	#drawfondoeti("fondoetiqueta.png", 55*mm, -45*mm, page_width, c) # irun p ANTES -39 VALENCIA
	drawfondoeti("fondoetiqueta2.png", 55*mm, -41*mm, page_width, c)
	#drawText("Filatelia", "FranklinGothic", 12, 1.5*mm, 19.5*mm, c) 
...
def printStamps(id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos):
	...
	for index, item in enumerate(items):
#----inicio ------------- AÑO 2022= SI <> FI IMPRIME SELLOS CON fecha_sello y evento_sello
		if item["cantidad"]>0 and  str(nombre_maquina[0:2]) != "FI":
			...
			if productos[index]["modo"] == "S":
				...
				for x in xrange(0,item["cantidad"]):
					if modelo==1:
						...
						genStampD(productos[index]["nombre_ticket"], fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cs[modelo-1][tarifa])
						id_producto = int(id_producto) + 1
						cs[modelo-1][tarifa].showPage()
				...
				if productos[index]["idProducto"] == "tarifaAT1" or productos[index]["idProducto"] == "tarifaAT2":
					...
					for x in xrange(0,item["cantidad"]):
						if modelo==1:	
							...
							genStampD("Tarifa A", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa A", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
					...
					for x in xrange(0,item["cantidad"]):
						if modelo==1:
							...
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa A2", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa B", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa C", fecha_sello, evento_sello, genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()		
#----inicio ------------- AÑO 2022= SI = FI IMPRIME SELLOS SIN fecha_sello y evento_sello
		elif item["cantidad"]>0 and  str(nombre_maquina[0:2]) == "FI":
			modelo = int(productos[index]["idProducto"][-1:])
			...
			if productos[index]["modo"] == "S":
				...
				for x in xrange(0,item["cantidad"]):
					if modelo==1:
						genStamp(productos[index]["nombre_ticket"], "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cs[modelo-1][tarifa])
						id_producto = int(id_producto) + 1
						cs[modelo-1][tarifa].showPage()
					else:
						genStampD(productos[index]["nombre_ticket"], "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), cs[modelo-1][tarifa])
						id_producto = int(id_producto) + 1
						cs[modelo-1][tarifa].showPage()
			elif productos[index]["modo"] == "T":
				if productos[index]["idProducto"] == "tarifaAT1" or productos[index]["idProducto"] == "tarifaAT2":
					...
					for x in xrange(0,item["cantidad"]):
						if modelo==1:
							genStamp("Tarifa A", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStamp("Tarifa A", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							...
							genStampD("Tarifa A", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa A", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
					...
					for x in xrange(0,item["cantidad"]):
						if modelo==1:
							...
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStamp("Tarifa A2", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStamp("Tarifa B", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStamp("Tarifa C", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
						else:
							genStampD("Tarifa A", "", "",genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa A2", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa B", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()
							genStampD("Tarifa C", "", "", genCodigo(modo_maquina, mes_maquina, pais_maquina, year_maquina, nombre_maquina, id_cliente, id_producto), ct[modelo-1])
							id_producto = int(id_producto) + 1
							ct[modelo-1].showPage()			
...
def printTickets(fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3):
	...
	#c.showPage()
	genTicketCaja(fecha_ticket, "", "", "", items, id_cliente, nombre_maquina, productos, feria, lugar, "", "", "", "", "", "", page_height, page_width, c)
	c.showPage()

	
	#elnombre_maquina = ''
	#elnombre_maquina = 'CH' + str(year_maquina)[-2:]
	
	# if nombre_maquina (es distinto a) ("CH" + year_maquina) O ("FI" + year_maquina) (ENTONCES IMPRIME TICKETS POR CADA TIRA)
	itemsColeccion = []
	for item in items:
	...
	for index, item in enumerate(itemsColeccion):
		...
		if items[index]["cantidad"]>0 and productos[index]["modo"] == "T" and str(nombre_maquina[0:2]) != "CH":
...
def afkarPrint(items, precios, id_cliente, id_producto, fecha_sello, evento_sello, fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, feria, lugar, empresa, cif, cp, l1, l2, l3):
	print "Printing"
	productos = [
		{
	...
	try:
		printTickets(fecha_ticket, modo_ticket, modo_ticket_copia, modelo1_ticket, modelo2_ticket, items, id_cliente, nombre_maquina, productos, feria, lugar, empresa, cif, cp, l1, l2, l3)
	except Exception as e:
		print e
	try:
		printStamps(id_cliente, id_producto, fecha_sello, evento_sello, modo_maquina, nombre_maquina, mes_maquina, pais_maquina, year_maquina, items, productos)
	except Exception as e:
		print e
