...
// PERFIL - PANTALLA
			...
			if (config.sello.elperfil ==1) {
				...
				this.limite = config.ticket.NUEVOlimiteImporte;
			}
...
// Pausar y reanudar impresora
pausarImpresora(){
	Meteor.call('pausarImpresora', function(error, result){
		if(error){
			alert('Error al pausar impresora: ' + error.reason);
		} else {
			alert('Impresora pausada correctamente');
		}
	});
}
reanudarImpresora(){
	Meteor.call('reanudarImpresora', function(error, result){
		if(error){
			alert('Error al reanudar impresora: ' + error.reason);
		} else {
			alert('Impresora reanudada correctamente');
		}
	});
}
imprimirFilatelia(){
...
imprimirSPDE(){
	...
	imprimir(){
		...
		if (this.perfilpros == "filatelia") {
				var eltitulo = "Filatelia de: " + this.config.ticket.titulo;
				var eltituloCopia = "COPIA Filatelia de: " + this.config.ticket.titulo;
			}
		if (this.perfilpros == "protocolo") {
				var eltitulo = "Protocolo de: " + this.config.ticket.titulo;
				var eltituloCopia = "COPIA Protocolo de: " + this.config.ticket.titulo;
			}
		if (this.perfilpros == "spde") {
				var eltitulo = "SPDE de: " + this.config.ticket.titulo;
				var eltituloCopia = "COPIA SPDE de: " + this.config.ticket.titulo;
			};	
...
// CAMPOS PARA REPORT.PY... IMPRIMIR... ARRAY NUMERADO
		...
		+ this.config.ticket.T1especial + "*¿?*"
		+ this.config.ticket.T2especial + "*¿?*"
		+ this.config.ticket.T3especial + "*¿?*"
		+ this.config.ticket.TEmod1 + "*¿?*"
		+ this.config.ticket.TEmod2 + "*¿?*"
		+ this.config.ticket.ImprimeCopiaTicket + "*¿?*"
		+ this.config.ticket.ImprimeMasterTicket + "*¿?*"
		+ eltituloCopia;
		...
		else if (this.total > this.config.sello.PERFILlimiteImporte) {
			alert("Ha excedido el límite de compra de " + this.config.sello.PERFILlimiteImporte+ "€");
		}
		...
		this.ticketsventa = 2 + this.tarifaAT1Cantidad + this.tarifa4T1Cantidad + this.tarifaAT2Cantidad + this.tarifa4T2Cantidad;
			...
			// JC: SEGÚN MODO NO RESTA TICKETS DEL CONTADOR... PAPEL ILIMITADO... if (this.elmodo!="Abono/Envío")
			if (this.elmodo=="Abono/Envío") {
				var modotickets = 0;
				//alert("MODO ABONO");
			}
			else if (this.elmodo=="Esporádicos") {
				var modotickets = 0;
				//alert("MODO ESPORADICOS");
			}
			else  { 
				var modotickets = 2 + this.tarifaAT1Cantidad + this.tarifa4T1Cantidad + this.tarifaAT2Cantidad + this.tarifa4T2Cantidad;		
				//alert("MODO DIFERENTE A ABONO Y ESPORADICOS");
			}
			let tickets = modotickets;
...
// INFORME: PERFIL *FILTRO*
			if (this.perfilpros == "filatelia") {
				orderProps.modoImpresion = "Filatelia de " + this.config.sello.elnperfil;
			}
			if (this.perfilpros == "protocolo") {
				orderProps.modoImpresion = "Protocolo de " + this.config.sello.elnperfil;
			}
			if (this.perfilpros == "spde") {
				orderProps.modoImpresion = "SPDE de " + this.config.sello.elnperfil;
			}
